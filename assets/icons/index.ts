import viewIcon from './icon-view.svg?raw'
import rightToggleIcon from './icon-right-arrow.svg?raw'
import downToggleIcon from './icon-down-arrow.svg?raw'
import shuffleIcon from './icon-shuffle.svg?raw'
import saveFileIcon from './icon-savefile.svg?raw'
import uploadIcon from './icon-upload.svg?raw'
import downloadIcon from './icon-download.svg?raw'
import cancelIcon from './icon-cancel.svg?raw'
import tickIcon from './icon-tick.svg?raw'
import homeIcon from './icon-home.svg?raw'
import historyIcon from './icon-history.svg?raw'
import graphIcon from './icon-graph.svg?raw'
import settingIcon from './icon-setting.svg?raw'
import lockIcon from './icon-lock.svg?raw'
import scrollIcon from './icon-scroll.svg?raw'
import boxIcon from './icon-box.svg?raw'
import packageIcon from './icon-package.svg?raw'
import plusIcon from './icon-plus.svg?raw'
import minusIcon from './icon-minus.svg?raw'
import crownIcon from './icon-color-crown.svg?raw'
import fireIcon from './icon-color-fire.svg?raw'
import shieldIcon from './icon-color-shield.svg?raw'
import gemIcon from './icon-color-gem.svg?raw'
import evilIcon from './icon-color-evil.svg?raw'
import smileIcon from './icon-color-smile.svg?raw'

export {
  viewIcon,
  rightToggleIcon,
  downToggleIcon,
  shuffleIcon,
  saveFileIcon,
  uploadIcon,
  downloadIcon,
  cancelIcon,
  tickIcon,
  homeIcon,
  historyIcon,
  graphIcon,
  settingIcon,
  lockIcon,
  scrollIcon,
  boxIcon,
  packageIcon,
  plusIcon,
  minusIcon,
  // color
  crownIcon,
  fireIcon,
  shieldIcon,
  gemIcon,
  evilIcon,
  smileIcon,
}
